...
setversioning() {
    # For staging branch
    KERNELNAME="ElectroPerf-$LINUXVER-P-WIFI-CAF-STABLE-X01BD-v2.1-$DATE"
...
gen_zip() {
	...
	cd AnyKernel3 || exit
	cp -af "$KERNEL_DIR"/init.ElectroSpectrum.rc init.spectrum.rc && sed -i "s/persist.spectrum.kernel.*/persist.spectrum.kernel ElectroPerf-P-WIFI-CAF-STABLE-v2.1/g" init.spectrum.rc
